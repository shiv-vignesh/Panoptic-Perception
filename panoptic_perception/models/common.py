...
class PatchEmbed(nn.Module):
    def __init__(self, image_size:Tuple[int, int]=(224, 224),
                in_channels:int=3, embed_dim:int=96, patch_size:int=4, drop_rate=0.,
                stride:int=4, activation:bool=False, batch_norm:bool=False, padding:int=0,
                apply_ape:bool=False, layer_norm:nn.LayerNorm=None
                ):
        ...
        patch_resolution = [image_size[0] // patch_size, image_size[1] // patch_size]
        self.num_patches = patch_resolution[0] * patch_resolution[1]
        self.patch_resolutions = patch_resolution
        ...
        if layer_norm is not None:
            assert isinstance(layer_norm, nn.LayerNorm), \
                f'Unsupported layer_norm isntance, expected {nn.LayerNorm} got {type(layer_norm)}'
        
        self.layer_norm = layer_norm

        if self.apply_ape:
            initial_weights = torch.empty(1, self.num_patches, embed_dim)
            nn.init.kaiming_normal_(initial_weights, mode='fan_in', nonlinearity='relu')
            self.ape_embed = nn.Parameter(initial_weights)

    def forward(self, x:torch.Tensor):
        B, C, H, W = x.shape

        assert H == self.image_size[0] and W == self.image_size[1], \
            f'Input image size ({H}*{W}) doesnt match model ({self.image_size[0]}*{self.image_size[1]}).'
...
class ShiftedWindowMSA(nn.Module):
    ...
    def __init__(self, embed_dim:int, window_size:int, 
                num_heads:int, qkv_bias:bool=True, qk_scale=None, 
    ...
    def _init_relative_pos_index(self):
        """
        ...
        # nn.init.kaiming_normal_(self.relative_position_index, mode='fan_in', nonlinearity='relu')

    def forward(self, x:torch.Tensor, mask:torch.Tensor=None):

        B, N, _C = x.shape #(B, w * h, C)
        
        #(B, w*h, C*3) --> (B, w*h, 3, num_heads, C // num_heads) 
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, _C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2] # (B, 3, w*h, C // num_heads)

        q = q * self.scale
        attn = (q @ k.transpose(-2, -1))

        relative_position_bias = self.relative_pos_bias_table[self.relative_position_index.view(-1)].view(
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, N, _C)
        x = self.proj(x)
...
class SwinBlock(nn.Module):
    ...
    def __init__(self, embed_dim:int, input_res:Tuple[int, int], 
                num_heads:int, window_size:Tuple[int, int]=(7, 7), shift_size=0, 
                mlp_ratio:float=4., act_layer=nn.GELU, norm_layer=nn.LayerNorm,
                qkv_bias: bool = True, qk_scale=None, 
                attn_dropout:float=0.0, drop=0., drop_path=0.,
        ...
        self.dropout_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        ...
        if self.shift_size:
            ...
            mask_windows = self._window_partition(img_mask)  # nW, window_size, window_size, 1
    ...
    def _window_reverse(self, windows, H, W):
        ...
        # (B * n_windows, num_pixels_per_window, embed_dim) - windows tensor
        B = int(windows.shape[0] // (H * W // self.window_size // self.window_size))
        # unwrap : (B, window_H * n_windows, window_w * n_windows, embed)
    ...
    def forward(self, x:torch.Tensor):
        ...
        assert N == H * W, f"expected num of pixels to be {H * W}, got {N}. Supported size: {self.input_res}"
        ...
        if self.shift_size > 0:
            if not self.fused_window_process:
                #(roll along spatial dim)
                shifted_x = torch.roll(x, shifts=(-self.shift_size, -self.shift_size), dims=(1, 2))
                x_windows = self._window_partition(shifted_x)

            else:
                raise NotImplementedError(
                    f'fused custom Window Partition operation that does more work in one step currently unsupported.'\
                    f'Retry with fused_window_process as False'
                )
        ...
        # (B * n_windows, num_pixels_per_window, embed_dim)
        ...
        if self.shift_size > 0:
            if not self.fused_window_process:
                shifted_x = self._window_reverse(attn_windows, H, W)  # B H' W' C
                x = torch.roll(shifted_x, shifts=(self.shift_size, self.shift_size), dims=(1, 2))
            else:
                raise NotImplementedError(
                    f'fused custom Window Partition Reverse operation that does more work in one step currently unsupported.'\
                    f'Retry with fused_window_process as False'
                )
        else:
            shifted_x = self._window_reverse(attn_windows, H, W)  # B H' W' C
            x = shifted_x # (B, window_H * n_windows, window_w * n_windows, embed)

        x = x.view(B, H * W, C)
        x = shortcut + self.dropout_path(x)
...
class PatchMerge(nn.Module):
    ...
    def __init__(self, input_res:Tuple[int, int], embed_dim:int, norm_layer=nn.LayerNorm):
    ...
    def forward(self, x:torch.Tensor):
        ...
        """
        Downsample the feature map by 4 and increase the capacity by 4
        """
        ...
        x = x.view(B, -1, 4 * _C) # (B, H//4, W//4, 4 * _C)
...
class SwinLayer(nn.Module):
    def __init__(self, embed_dim:int, input_res, depth, num_heads, window_size, 
                mlp_ratio:float=4., qkv_bias: bool = True, qk_scale=None, 
                attn_dropout:float=0.0, drop=0., drop_path=0., 
                norm_layer=nn.LayerNorm, downsample:PatchMerge=None,
        ...
        if downsample is not None:
            assert issubclass(downsample, PatchMerge), \
                f'Currently supported downsample {PatchMerge}, got {type(downsample)}'
            self.downsample = downsample(input_res=input_res, embed_dim=embed_dim, norm_layer=norm_layer)
        else:
